...
- 画面: タイトルに名前入力（`#pname`）と「ランキング」（`#rankbtn`）、リザルトに上位 5 件（`#res-rankbox`）。
  **ランキング画面は 10 位まで**（2026-09-12 ユーザー指示。以前は 20 位）。自分が外れていたら「…」のあとに自分の行を足す
- **端末内の順位表は持たない**（2026-09-12 にユーザー判断で撤去）。みんなのランキングがあるので不要。
  以前は通信の待ち時間に端末内の記録を出していて、自分の過去プレイが 10 件並び「みんなのランキング」に見えていた
  - 残しているのは **名前ごとの自己ベスト（`enbu.best`）だけ**。NEW RECORD の判定に使う。
    昔の一覧（`enbu.rank`）は読み込み時に消す
  - 取れるまでは `showRankBoard([], ..., loading=true)` で「読み込んでいます…」。
    取れなければ「通信できませんでした」。リザルトでは `UI.setResultNote()` で一言だけ出す
  ランキング画面は `#rankboard` / `#rank-panel`。**`#res-panel` と id を分けること**（最初に同じ id を 2 つ置いてしまった）
- 記録するのは**勝ったときだけ**。負けは載せない。
  負けたリザルトには順位表の代わりに「クリアしていないので、この記録はランキングに登録されません。」を出す（`.rank-lose`）
...
- **両キャラとも 5 段目は `lunge: 0`**（まひろのジャンプ回し蹴り、ちさとの回転。どちらも当たりにくいと指摘された）。
  ちさとの `C_WHIRL` は hipsY を上げて**その場で跳ぶ**形にした。前へ出ると敵を通り過ぎて当たらないという指摘への対応。
...
- 流れ（`SUP_CUTIN` 1.0 秒 → `SUP_DASH` 0.75 秒 → `SUP_AFTER` 1.2 秒）:
  ...
  - **2026-09-13 に動きを足した**（「もっとダイナミックに」）:
    - **カメラが溜めのあいだ 0.75 ラジアン回り込む**。止まった絵にならないように
    - 当てた瞬間に距離 3.0m までぐっと寄り、0.7 秒かけて引く。注視点も敵側へ 55% 寄せる
    - 踏み切りで足元に衝撃波と土煙。飛んでいる間は 0.045 秒ごとに地面へ輪を置いて速さを見せる
    - **当てたあとも止まらず 2.6m 突き抜ける**。敵は 0.32 秒かけて押し戻す
      （位置を少しずつ動かすので髪は暴れない）
...
` で改行が効く。画面下 18vh に置いて二人に被らないようにした。
  **集中線（斜線）は入れていた が、邪魔だとの指摘で撤去した**（2026-09-12）。文字だけ
- ボスは `Boss.freeze(true, ctx)` で停止。パターン進行と移動を止め、溜めていた弾も消す
- **必殺技で決着がつくことがある**。`finish()` の先頭で `endSuper()` を呼ぶこと。
  体を元の入れ物へ戻さないとリザルトでメインが消える（2026-09-12 に指摘されて修正）
- 見た目は参考画像に寄せてある: 溜めで沈み、踏み切って**宙に浮き、前を上げた斜めの姿勢で飛ぶ**。
  `poseCarry(t, fly)` / `poseRam(t, fly)` の fly が 0→1 で、脚が後ろへ流れる。
  **まひろは肩に担ぐ**（`position (-0.16, 1.34, -0.52)`、`rotation (π/2, 0, -0.12)`）。足は後ろ、頭だけ前へ出る。
  ちさとは右腕を上げて押さえ、左腕を前へ伸ばす。
  **打つのは頭なので、まひろの拳は前へ出さない**（2026-09-12 ユーザー指示）
  - 高さは `group.position.y`（沈み -0.12 → 跳躍 +0.62）、傾きは `group.rotation.x = -0.3`
  - **カメラの注視点も高さに追従させること**。追従しないと二人が画面から外れる
- 検証は `?super&spf=コマ数`（ゲージを満タンにして即発動。着弾は 100 コマ前後）。
  必殺技で倒すところまで見るなら `?hp=150&super&spf=560`

## タイトルで BGM を鳴らす（2026-09-12）

- **キャラ選択の画面が出た時点で鳴らしにいく**（`tryStartMusicNow()`）。ユーザー要望
- ブラウザは一度も触られていないページの音を止めるので、開けなければ
  `pointerdown` / `keydown` / `touchstart` を一度だけ拾って開き直す。**どこを触っても鳴る**
- 開けないあいだはタイトルに「画面に触れると音が鳴ります」を出す（`UI.setAudioHint`）。開けたら消す
- 音が止まっていても AudioContext と曲の読み込みは先に始まるので、触った瞬間に鳴り出す
...
## 配点の調整（2026-09-13、まず試遊版へ）

- 調査で分かった歪み: 点の大半が**打撃の回数**で決まるため、弱い攻撃を当て続けるほうが得で、速攻が損をしていた
- 入れた対処（ユーザー了承の 1 と 2）:
  1. **タイムボーナスを 10 倍**（1分以内 1000万 / 2分以内 500万 / 3分以内 200万）
  2. **必殺技の命中に 1 回 200万**（`SUPER_PT`）。`Player.superHits` で数える
- 手元の計算での比較（体力 800 前提）:
  - 速攻型（53発・ノーダメ・52秒・コンボ60・必殺2）→ 約 1 億 1000万（神人間に届く）
  - じっくり型（弱攻撃 130発・200秒・コンボ80）→ 約 1 億 5000万
  - **まだ「じっくり弱攻撃」が最上位**。順位を逆転させるには案 3（与ダメージ配点）が必要
- 検証は `?win=42&sec=52&mc=60&mp=4&bp=20&sh=2`（`sh` が必殺技の命中回数）

## 負けたときの相棒（2026-09-13、まず試遊版へ）

- 倒れた相棒の隣に**しゃがんで手を取る**（`poseKneelHold`）。左手を下へ伸ばす。
