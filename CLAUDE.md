...
- **共有ボタンは廃止**（2026-09-12。ユーザー判断「画面キャプチャ取ればいいし」）。リザルトのボタンは「もう一度」だけ。
  文面を作る `shareText()` / `shareUrl()` は `?sharetest` の検証用に残してある。
  経緯: 端末の共有画面はリンクだけを渡す先が多く点数が消える → X の投稿画面を直接開く形にした → 不要と判断された
...
- `public/audio/voice/` に `voice01.mp3` から順に置くと、リザルトで 1 本を無作為に鳴らす。
  **置いた分だけ候補になる**（起動時に voice01〜voice24 の有無を HEAD で調べる）。連続で同じものは出さない
  - 中身の読み込みは鳴らす直前。一度読んだものは覚える。音量は `VOICE_GAIN`（2026-09-12 に 0.9 → 1.5）。
    **これ以上上げると BGM と重なったとき歪む**
  - **スコア画面では無作為に 2 本まで**（`startVoiceLoop(遅れ, 本数, 間)`）。1 本終わるごとに 1.1 秒あけて次を選ぶ。
    `restart()` で `stopVoiceLoop()` を呼んで止める。世代番号で古い繰り返しを無効にしている
  - 最初の 1 本は勝利の音と重ならないよう 0.9 秒遅らせる
...
## BGM が始まるまで無音だった（2026-09-12 修正）

- 症状: 曲がしばらく鳴らず、遊んでいる途中で「急に聞こえた」
- 原因: 曲が **4.87MB** あり、落として展開し終わるまで何も鳴らしていなかった。打撃音は合成音なので先に鳴る
- 対処:
  1. **ページを開いた時点で曲の中身を落とし始める**（Bgm.ts の `prefetch()`）。
     音を鳴らせるのは画面に触れてからだが、ダウンロードは待つ必要がない
  2. つなぎの合成 BGM は**やめた**（2026-09-12。「古い BGM が少し流れる」→「不要」とユーザー判断）。
     合成 BGM は mp3 が読めなかったときの受け皿としてだけ残っている

## ♪ を入れ直しても鳴らなかった（2026-09-12 修正）

- 症状: BGM を切る → ブラウザを開き直す → ♪ で入れ直しても無音。**キャラを選んだ瞬間に鳴り出す**
- 原因: **♪ を押しても音を開けていなかった**。AudioContext は操作がないと開けず、`onGesture` は
  キャラ選択からしか呼ばれていなかった。♪ は押せても、音の土台が無いので BGM が作られない
- 対処: ♪ の処理からも `onGesture()` を呼ぶ。ただし**戦闘中に押されたとき濃さを 0 に戻さない**よう、
  Game 側は `state === 'title'` のときだけ濃さを 0 にする
- 併せて直した別の不具合: `Bgm.apply()` が**読み込みを待つ前に音量を決めていた**ため、
  待っている間に ♪ を押されると古い値（無音）で上書きされていた。音量は `get vol` で使う直前に求める
...
## 名前の予約語（2026-09-12 に入れて同日に撤去）

- 「管理者」を名乗れないようにする仕組みを入れたが、**ユーザー判断で削除した**（「みんな自由に入力するようにしよう。ただの遊びだし」）。
  再度求められたら履歴から戻せる。合言葉つき URL で端末を解錠する形だった

## 合体必殺技「スーパー！まひろ頭突き！」（2026-09-12）

- **ちさとがまひろを抱えて頭から突っ込む**。ユーザー提供の参考画像の構図。技の間ボスは何もできず、必ず当たる
- ゲージ: Player の `superGauge` 0..1。打撃 0.016 / 突進パリィ 0.07 / 弾パリィ 0.03 / 被弾 0.08。
  殴るだけでは 63 発かかるので、受け流しと被弾も混ぜないと満タンにならない（ユーザー要望「溜まるまで少し大変さを」）
- 操作: 満タンのとき `#b-super`（必／U キー）が光って出る。`Input` の Action に `super` を足した
- 流れ（`SUP_CUTIN` 1.0 秒 → `SUP_DASH` 0.75 秒 → `SUP_AFTER` 1.2 秒）:
  1. `startSuper()` が二人の rig を**一時的な Group に付け替える**。
