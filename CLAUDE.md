...
- **読み込み中に端末内の記録を出さないこと**。以前は通信の待ち時間に local を出していたため、
  自分の過去プレイが 10 件並び「みんなのランキング」に見えていた（2026-09-12 に指摘されて修正）。
  いまは `showRankBoard([], ..., loading=true)` で「読み込んでいます…」だけ出し、取れてから差し替える
