...
export type TrailPart = 'fist' | 'knife' | 'leg';
...
/** 深川まひろ: 重い。ジャブ → ナイフのフック → アッパー → ハイキック → ジャンプ回し蹴り */
const MAHIRO: FightStyle = {
  id: 'mahiro',
  attacks: {
    1: { total: 0.32, a0: 0.1, a1: 0.16, chain: 0.17, dmg: 7, poise: 10, lunge: 5.5, reach: 1.9, kind: 'punch', trail: 'fist' },
    2: { total: 0.4, a0: 0.15, a1: 0.22, chain: 0.23, dmg: 10, poise: 14, lunge: 5.5, reach: 2.0, kind: 'knife', trail: 'knife' },
  ...
  sharp: false,
  swing(sfx, kind) {
    if (kind === 'knife') sfx.slashHeavy();
    else sfx.whooshHeavy(kind === 'spin' ? 0.6 : kind === 'kick' ? 0.75 : 0.95);
  },
  hit(sfx, kind, heavy) {
...
/** 杉本ちさと: 鋭い。刺突ジャブ → ナイフの斜め切り下ろし → 切り上げ → 前蹴り → 回転切り */
const CHISATO: FightStyle = {
  id: 'chisato',
  attacks: {
    1: { total: 0.26, a0: 0.08, a1: 0.13, chain: 0.14, dmg: 5, poise: 7, lunge: 6.0, reach: 1.9, kind: 'punch', trail: 'fist' },
    2: { total: 0.32, a0: 0.11, a1: 0.17, chain: 0.19, dmg: 8, poise: 10, lunge: 6.0, reach: 2.1, kind: 'knife', trail: 'knife' },
    3: { total: 0.32, a0: 0.11, a1: 0.17, chain: 0.19, dmg: 8, poise: 10, lunge: 6.0, reach: 2.1, kind: 'knife', trail: 'knife' },
    4: { total: 0.4, a0: 0.15, a1: 0.23, chain: 0.26, dmg: 11, poise: 16, lunge: 5.0, reach: 2.3, kind: 'kick', trail: 'leg' },
    5: { total: 0.62, a0: 0.28, a1: 0.42, chain: 0.62, dmg: 22, poise: 36, lunge: 9.0, reach: 2.6, kind: 'spin', trail: 'knife' },
  },
  ...
  sharp: true,
  swing(sfx, kind) {
    if (kind === 'knife' || kind === 'spin') sfx.slashSharp(kind === 'spin' ? 0.85 : 1);
    else sfx.whooshSharp(kind === 'kick' ? 0.9 : 1.15);
  },
  hit(sfx, kind, heavy) {
    sfx.ping(kind === 'knife' || kind === 'spin' ? 1.2 : 1, heavy);
  },
};
