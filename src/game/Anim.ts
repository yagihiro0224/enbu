...
/**
 * 相棒を**肩に担いで**跳ぶ姿勢（ちさと）。
 * 右腕は上げて相棒の体を押さえ、左腕は前へ出して balance を取る。
...
export function poseCarry(t: number, fly = 0): Pose {
  ...
  const spine: Vec3 = [0.2 + 0.12 * k, 0, -0.06];
  return {
    ...arms(
      {
        // 右は担いでいる体を上から押さえる、左は前へ伸ばす
        R: [[-0.3, 0.86, 0.4], [-0.2, 0.52, 0.83]],
        L: [[0.42, 0.1, 0.9], [0.2, 0.16, 0.96]],
      },
      spine
    ),
    hipsY: -0.06 + bob,
    spine,
    head: [-0.16 - 0.08 * k, 0.12, 0],
...
/**
 * 肩に担がれて頭から突っ込む姿勢（まひろ）。
 * 体は親側で横倒しにするので、ここでは腕を体へ沿わせて後ろへ流し、頭だけ突き出す。
 * **打つのは頭なので、拳は前へ出さない**（2026-09-12 ユーザー指示）
 */
export function poseRam(t: number, fly = 0): Pose {
  ...
  return {
    ...arms({
      // 両腕とも体に沿わせて後ろへ。担がれているので力は抜けている
      L: [[0.36, -0.24, -0.88], [0.16, -0.18, -0.96]],
      R: [[-0.36, -0.24, -0.88], [-0.16, -0.18, -0.96]],
    }),
    spine: [-0.1 - 0.06 * k + q, 0, 0],
    head: [-0.36, 0, 0],
    upperLegL: [0.2 + 0.16 * k, 0, -0.06],
    lowerLegL: [0.18 + q, 0, 0],
    upperLegR: [0.12 - 0.08 * k, 0, 0.06],
    lowerLegR: [0.34 - q, 0, 0],
  };
}
...
/** ちさとの 5 段目。**その場で跳んで回る**（前に出ると通り過ぎて当たらない） */
const C_WHIRL: [number, Pose][] = [
  [0, GUARD],
  key(0.22, [0.45, -0.6, 0], { L: [[0.5, -0.7, 0.3], [0.1, -0.2, 0.95]], R: [[0.3, -0.85, 0.3], [0.7, -0.5, 0.5]] }, { hipsY: -0.26, head: [-0.2, 0.4, 0], upperLegL: [-0.7, 0, -0.1], lowerLegL: [1.2, 0, 0], upperLegR: [-0.2, 0, 0.1], lowerLegR: [1.0, 0, 0] }),
  key(0.4, [0.15, 0, 0], { L: [[0.5, -0.6, -0.5], [0.3, -0.3, -0.9]], R: [[-0.95, -0.05, -0.2], [-0.98, -0.05, 0.1]] }, { hipsY: 0.42, head: [0, 0, 0], upperLegL: [-0.5, 0, -0.15], lowerLegL: [0.9, 0, 0], upperLegR: [0.1, 0, 0.2], lowerLegR: [0.7, 0, 0] }),
  key(0.6, [0.15, 0, 0], { L: [[0.5, -0.6, -0.5], [0.3, -0.3, -0.9]], R: [[-0.95, -0.05, 0.2], [-0.98, -0.05, 0.15]] }, { hipsY: 0.5, head: [0, 0, 0], upperLegL: [-0.5, 0, -0.15], lowerLegL: [0.9, 0, 0], upperLegR: [0.1, 0, 0.2], lowerLegR: [0.7, 0, 0] }),
  key(0.8, [0.3, -0.3, 0], { L: [[0.4, -0.7, 0.5], [-0.2, 0.6, 0.7]], R: [[0.5, -0.4, 0.75], [0.85, -0.3, 0.4]] }, { hipsY: 0.08, head: [-0.1, 0.2, 0], upperLegL: [-0.6, 0, -0.1], lowerLegL: [1.0, 0, 0], upperLegR: [0.2, 0, 0.1], lowerLegR: [0.8, 0, 0] }),
  [1, GUARD],
];
